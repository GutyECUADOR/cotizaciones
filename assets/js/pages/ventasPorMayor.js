...
const app = new Vue({
    el: '#app',
    data: {
        titulo: 'Transacciones de Venta - Pedido al por mayor',
        fechasPago: [],
        search_documentos: {
            busqueda: {
    ...
    methods:{
        ...
        async getCliente() {
            ...
            this.search_cliente_response = response;
            this.getFechasPago(this.search_cliente_response?.data.NUMPAG,  this.search_cliente_response?.data.ENTREPAG);
            $('#modalAceptaCondicionesVentaXMayor').modal('show');
            
        },
        async selectCliente() {
            let RUC = this.search_cliente.busqueda.texto;
            const response = this.search_cliente_response;
            ...
            const response_docpendientes = await this.getDocsPendientesCliente(response.data.CODIGO);
           
            if (response_docpendientes && response_docpendientes.Cantidad > 0) { 
                swal({
                    title: "Documentos Pendiente!",
                    text: `El cliente con el RUC: ${RUC} tiene ${response_docpendientes.Cantidad} documentos pendientes. Comuníquese con el departamento de crédito.`,
                    type: "warning",
                    showCancelButton: false,
                    confirmButtonClass: "btn-success",
                    confirmButtonText: "Aceptar",
        ...
        async saveDocumento(){
            ...
            if (response.commit) {
                ...
                swal({
                    ...
                    function(){
                        window.location = './index.php?action=ventasPorMayor'
                    });
            }else {
                console.log(response);
                swal({
        ...
        getFechasPago(cantidadPagos, diasEntrePagos) {
            let cantidad_Pagos = parseInt(cantidadPagos)
            let diasEntre_Pagos = parseInt(diasEntrePagos)
            
            if (cantidad_Pagos && diasEntre_Pagos) {
                let fechasPago = [];
                let currentdate = moment(this.documento.fecha, "YYYY-MMD-DD");

                for (let index = 1; index <= cantidadPagos; index++) {
                    fechasPago.push(currentdate.add(30, 'days').format("YYYY-MM-DD"));
                }
                this.fechasPago = fechasPago;
            }else {
                let currentdate = moment(this.documento.fecha, "YYYY-MM-DD").add(30, 'days').format("YYYY-MM-DD");
                this.fechasPago = [currentdate];
            }
        }
        
    },
    filters: {
        tiposPago: function (value) {
            switch (value) {
                case 'CRE':
                    return 'CREDITO'
                    break;
