...
class Producto {
  constructor({codigo, nombre, marca, nuevaMarca}) {
      this.codigo = codigo;
      this.nombre = nombre;
      this.marca = marca;
...
const app = new Vue({
    el: '#app',
    data: {
      title: 'Actualizacion de Marcas | Productos',
    ...
    methods:{
      init() {
        fetch(`./api/inventario/index.php?action=getInfoInitForm_actualizarMarcaProducto`)
          .then(response => {
            return response.json();
          })
          .then(result => {
            console.log('InitForm', result.data);
            const { databases, marcas } = result.data;
            ...
            fileReader.onload = (event) => {
              ...
              try {
                ...
                rows.forEach( (rowExcel) => {
                  ...
                  if (!rowExcel.NUEVAMARCA) {
                    throw new Error(`La nueva marca del Producto ${rowExcel.CODIGO}, esta vacia o es invalida`);
                  }
                  ...
                  if (producto) {
                    ...
                    let newProduct = new Producto({
                      ...
                      nuevaMarca: rowExcel.NUEVAMARCA
                    });
      ...
      saveProducts() {
        ...
        this.documento.databases.forEach(async database => {
          ...
          const data = await fetch(`./api/inventario/index.php?action=updateMarcaProductos`, {
            method: 'POST',
            body: formData
          })
            .then(response => {
